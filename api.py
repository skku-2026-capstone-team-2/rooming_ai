...
class RecommendRequest(BaseModel):
    ...
    user_places: list[UserPlace] = Field([], description="자주 가는 장소 목록")
...
def recommend(req: RecommendRequest):
    frequent_dicts = [{"name": p.name, "lat": p.lat, "lng": p.lng} for p in req.user_places]
    frequent_names = [p.name for p in req.user_places]

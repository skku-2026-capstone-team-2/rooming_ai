...
class RecommendRequest(BaseModel):
    ...
    seeker_id:   Optional[int] = Field(None, description="매물 탐색자 ID (자주 가는 장소 조회용)")
...
def recommend(req: RecommendRequest):
    ...
    for step, status, message, data in search_stream(
        req.query, req.top_n, req.seeker_id, req.preferences
    ):
        if status == "error":
            error_msg = message
            break
        if step == 5 and status == "done" and data:
            frequent_names = data.get("frequent_names", [])
